...
func init() {
	mItems = map[int]mItem{
		1:  mItem{1, 0, 1, 0, 1, 0, 1, 1, 1},
		2:  mItem{2, 0, 1, 1, 1, 0, 1, 2, 1},
		3:  mItem{3, 1, 10, 0, 2, 1, 3, 1, 2},
		4:  mItem{4, 1, 24, 1, 2, 1, 10, 0, 3},
		5:  mItem{5, 1, 25, 100, 3, 2, 20, 20, 2},
		6:  mItem{6, 1, 30, 147, 13, 1, 22, 69, 17},
		7:  mItem{7, 5, 80, 128, 6, 6, 61, 200, 5},
		8:  mItem{8, 20, 340, 180, 3, 9, 105, 134, 14},
		9:  mItem{9, 55, 520, 335, 5, 48, 243, 600, 7},
		10: mItem{10, 157, 1071, 1700, 12, 157, 625, 1000, 13},
		11: mItem{11, 2000, 7500, 2600, 3, 2001, 5430, 1000, 3},
		12: mItem{12, 1000, 9000, 0, 17, 963, 7689, 1, 19},
		13: mItem{13, 11000, 11000, 11000, 23, 10000, 2, 2, 29},
	}
}
...
func buyItem(roomName string, itemID int, countBought int, reqTime int64) bool {
	...
	for _, b := range buyings {
		var item mItem = mItems[b.ItemID]
...
func (sm *statsMap) get(room string, currentTime int64) *Glock {
	sm.mu.Lock()
	defer sm.mu.Unlock()
	key := fmt.Sprintf("%s:%d", room, currentTime)
	_, ok := sm.ma[key]
	if !ok {
		sm.ma[key] = &Glock{roomName: room, currentTime: currentTime}
	}
	return sm.ma[key]
}
...
func getStatus(roomName string) (*GameStatus, error) {
	...
	status, err := sm.get(roomName, currentTime).get(tx)
	if err != nil {
		return nil, err
	}
...
func (gl *Glock) get(tx *sqlx.Tx) (*GameStatus, error) {
	gl.mu.Lock()
	defer gl.mu.Unlock()
